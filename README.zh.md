...
为 LLM 提供长期记忆的层，实现 **ENGRAM v2.1**（[`SPEC.md`](SPEC.md)，日文）：一个源自人类记忆原理的**记忆痕迹（trace）模型**，仅依赖**本地嵌入模型**（EmbeddingGemma GGUF，经 llama.cpp 运行）和**单文件 SQLite**。LLM 只在三个时刻生成：写入、使用（并引用）、以及离线的**梦**（整合）。
...
| 动词 | 行为 |
|---|---|
| `remember(text, salience, cue)` | 完全相同的文本视为复述；否则插入，**从不覆盖**。`cue` 触及邻居（cos ≥ θ_related）的痕迹以*不稳定*状态诞生；已有痕迹不会被改动。`cue` 是这条事实日后会被问到的问题，也是梦阶段用来检索过去的线索，因此即使更新与被更新的两句话字面相去甚远，更新也能找到它所取代的那一版。超出容量时，遗忘宽限期（3 天）之外强度最低的痕迹。 |
| `recall(query)` | 多线索余弦 → `score = a·(α + (1−α)·R)` → 绝对/相对阈值 → MMR → 以 `[unix tz] text 《id》` 注入 ≤1024 字。注入是"暴露"，只强化一半。 |
| `cite(reply)` | LLM 引用的《id》记忆按"使用"完整强化。 |
| `forget(id)` | 按 id 物理删除。 |
| `dream(budget)` | 不稳定痕迹（先进先出，≤ 8·budget）作为种子，取其 `cue` 所再激活的**更早**痕迹构成簇（cos ≥ θ_related，≤8），由你的 LLM 判定 **keep** 或 **replace**（被取代的旧记忆 id ＋ 要旨文本）。要旨继承最强成员的稳定度加其他成员的"活证据"；与输入无关的输出被视为虚构而拒绝。已整理的存储不会调用 LLM。 |
