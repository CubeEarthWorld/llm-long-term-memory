...
def test_three_thousand_years(make_system, cfg):
    ...
    # theta_related is calibrated per embedding model; the token-overlap fake scores
    # unrelated text far higher than a real model, so the reactivation band is raised
    # here to keep the same selectivity (mirrors the Dart longevity test).
    cfg.memory.theta_related = 0.75
    ...
    for m in rows:
        ...
        assert 0 <= r <= 1                             # a NaN already fails this
    for f in facts:
